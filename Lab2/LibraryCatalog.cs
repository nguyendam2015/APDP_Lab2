...
namespace Lab2
{
    ...
    public class LibraryCatalog
    {
        ...
        public void ListItems()
        {
            ...
            {
                Console.WriteLine($"{item.GetType().Name} by {item.Title} by {item.Author} - Available: {item.Available}");
            }
        }
        ...
        public void FindItem(string keyword)
        {
            ...
            ||i.Author.Contains(keyword, StringComparison.OrdinalIgnoreCase)
            ).ToList();

            if (found.Count == 0)
            {
                Console.WriteLine($"No items found matching '{keyword}'.");
                return;
            }
            foreach (var item in found)
            {
                ...
                {
                    Console.WriteLine($"{item.Title} directed by {item.Author} - Runtime: {(item as DVD).Runtime} mins - Available: {item.Available}");
                }
                else
                {
